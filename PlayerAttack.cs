...
public class PlayerAttack : MonoBehaviour
{
    ...
    void Start(){
        ...
        current_Combo_State = ComboState.NONE; // not doing anything when starting
    }
    ...
    void ComboAttacks(){
        if(Input.GetKeyDown(KeyCode.Z)){
            ...
            current_Combo_State++; //Iterating through ComboState
        ...
        if(Input.GetKeyDown(KeyCode.X)){
            //if current combo is punch3 or kick2 exit - no combos left
            if(current_Combo_State == ComboState.KICK_2 || current_Combo_State == ComboState.PUNCH_3){
                return;
            }
            //if current combo state is NONE/ punch1 or punch2 set current combo state to kick 1
